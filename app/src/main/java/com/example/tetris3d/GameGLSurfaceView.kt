...
class GameGLSurfaceView(context: Context) : GLSurfaceView(context) {
    ...
    private fun startDropLoop() { stopDropLoop(); isRunning = true; scheduleNextDrop() }
    private fun stopDropLoop() { isRunning = false; gameHandler.removeCallbacksAndMessages(null) }
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        when (event.action) {
            ...
            MotionEvent.ACTION_UP -> {
                ...
                when {
                    absDx < minDistance && absDy < minDistance -> game.rotate()
                    absDx > absDy -> { if (deltaX > 0) game.moveRight() else game.moveLeft() }
                    else -> { if (deltaY > 0) game.hardDrop() else game.rotate() }
                }
                requestRender()
            }
        }
    ...
    override fun onPause() { super.onPause(); stopDropLoop() }
    override fun onResume() { super.onResume(); if (!isRunning && !game.isGameOver) startDropLoop() }
}
