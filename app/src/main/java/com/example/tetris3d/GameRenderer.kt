...
class GameRenderer(private val game: TetrisGame) : GLSurfaceView.Renderer {
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        for (pos in shape) {
            val px = game.currentX + pos.x
            val py = game.currentY + pos.y
            if (py in 0..19 && px in 0..9) {
                drawBlock(px, py, game.currentColorType)
            }
        }
    }
    ...
    private fun drawBlock(x: Int, y: Int, colorType: Int) {
        ...
        val color = when (colorType) {
            1 -> floatArrayOf(0.0f, 0.9f, 0.9f, 1.0f)
            2 -> floatArrayOf(0.9f, 0.9f, 0.0f, 1.0f)
            3 -> floatArrayOf(0.7f, 0.0f, 0.7f, 1.0f)
            4 -> floatArrayOf(0.9f, 0.5f, 0.0f, 1.0f)
            else -> floatArrayOf(0.0f, 0.9f, 0.0f, 1.0f)
        }
        cube.draw(scratch, color)
    }
